// v0.30 農莊68個新增 Canonical GLB 生成器。
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { FARM_EXPANSION_ASSET_DEFINITIONS } from '../src/data/farmExpansionAssetCatalog.js';
...
const results = [];
for (const definition of FARM_EXPANSION_ASSET_DEFINITIONS) {
  const asset = getCanonicalAsset(definition.assetId);
  if (!asset) throw new Error(`農莊資產未註冊：${definition.assetId}`);
...
console.log(JSON.stringify({ scope: 'farm-expansion', generatedCount: results.length, results }, null, 2));
